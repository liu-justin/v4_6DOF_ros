...
class Trajectory():
    ...
    def appendFirst(self, new_time, new_point):
        m_x, b_x = linear_init(self.points[-1][0], new_point[0], self.times[-1], new_time)
        b_y, c_y = poly_init(self.points[-1][1], new_point[1], self.times[-1], new_time)
        m_z, b_z = linear_init(self.points[-1][2], new_point[2], self.times[-1], new_time)

        print(f"Appending first: {m_x}, {b_x}, {b_y}, {c_y}, {m_z}, {b_z}")
    ...
    def append(self, new_time, new_point):
        if not self.constants_established:
            ...
            predicted_x = self.beta0_x + self.beta1_x*new_time
            predicted_y = self.beta0_y + self.beta1_y*new_time + -9.81*(new_time**2)
            predicted_z = self.beta0_z + self.beta1_z*new_time
